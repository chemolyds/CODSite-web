...
const EditAbout = (props) => {
	...
	const [URL, setURL] = useState(props.about.url);
	const [thumbnail, setThumbnail] = useState(props.about.thumbnail);
	const [header, setHeader] = useState(props.about.header);
	const [description, setDescription] = useState(props.about.description);
	...
	const handleSubmit = (event) => {
		...
		if (payload.url) {
			axios.put(`/api/about/edit_about/${props.about._id}`, payload)
				.then((res) => {
					setShow(false);
		...
		<>
			...
			<Modal show={show} onHide={handleClose} dialogClassName="modal-lg">
				<Modal.Header closeButton>
					<Modal.Title>Editing About</Modal.Title>
				</Modal.Header>
